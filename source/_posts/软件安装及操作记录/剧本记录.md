...
### 1、安装部署 Kubernetes 计算节点

```bash
[root@ecs-proxy ~]# mkdir node_install
[root@ecs-proxy ~]# cd node_install
# 拷贝控制节点上的 config.toml 作为模板文件 config.j2
[root@ecs-proxy node_install]# scp master: /etc/containerd/config.toml ./config.j2
[root@ecs-proxy node_install]# vim ansible.cfg
[defaults]
inventory         = hostlist
host_key_checking = False
[root@ecs-proxy node_install]# vim hostlist
[nodes]
192.168.1.[51:55]
```
